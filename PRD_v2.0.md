...
### Version 2.0 | MVP | April 2026

---

> **For AI coding agents:** This document is the product specification. Implementation is handled by OpenClaw + Kimi K2.6. Do not deviate from scope, data models, or acceptance criteria defined here without explicit instruction. Companion documents in this suite: TECH_SPEC.md, ARCH_DESIGN.md, QUALITY_SPEC.md, SECURITY_COMPLIANCE.md, RELIABILITY_DEVOPS.md, UX_SPEC.md.
...
This system is built by an AI coding agent, not a human developer. The toolchain is:

- **Agent runtime:** OpenClaw (self-hosted, local)
- **LLM backbone:** Kimi K2.6 (model-agnostic via OpenClaw config)
- **Validation:** Claude Sonnet 4.6 (RAG prompt design, financial edge cases, final benchmark validation)
- **Human role:** Domain validation only — confirming extraction accuracy against real BSE data, not writing code

**Implications for the agent:**
- All code must be runnable end-to-end without human intervention in the execution environment
- Each module must include a `__main__` block with a self-test against sample data
- Extraction modules must log their own quality metrics to a `pipeline_runs` table on every execution
- No interactive prompts — all configuration via environment variables or config.py
- Every script must be idempotent — safe to re-run on the same data
...
1. Analyst opens IndiaIR home page
2. Types keyword(s) into search bar (e.g. "capacity utilization")
3. Optionally selects company filter (multi-select), quarter range, document type, speaker role
4. Hits Enter or clicks Search
5. Results load in < 2 seconds: list of passages with company, quarter, speaker, snippet
6. Analyst clicks a result → transcript viewer opens, scrolled to that passage, keyword highlighted
7. Analyst can copy passage or click through to original BSE filing PDF

**Empty state:** "No results for [query]. Try broader terms or adjust your filters."
**Error state:** "Search unavailable. Please try again in a moment." (ES down)

### Flow 2 — RAG Q&A
1. Analyst opens Q&A page
2. Optionally pins company scope (e.g. "Laurus Labs only") and/or date range
3. Types natural language question
4. Streaming answer appears within 5 seconds, with inline citations
5. Financial context panel on right shows relevant quarterly P&L if the question touches financials
6. Analyst clicks a citation → transcript viewer opens at that passage
7. Analyst can ask a follow-up question in the same session (2-turn context maintained)

**Empty state (no relevant context found):** Answer states "I could not find relevant passages for this question in the current coverage universe." — does not hallucinate.
**Error state:** "Unable to generate answer. Claude API unavailable." — does not show partial response.

### Flow 3 — Company Page
1. Analyst selects a company from the coverage list
2. Company page loads: financials chart (revenue + EBITDA margin, last 20 quarters), management changes panel, event timeline
3. Analyst clicks a quarter on the financials chart → event panel for that quarter expands
4. Analyst clicks an event → options to open transcript viewer, open investor presentation, or read press release
5. Management changes panel shows KMP/Board timeline with effective dates

### Flow 4 — Management Changes View
1. Analyst opens Management Changes page
2. Default view: all KMP changes across coverage universe, last 24 months, sorted by date descending
3. Filters: company, role category, change type, date range
4. Each row shows: company, person, role, change type, effective date, reason (if stated)
5. Clicking a row expands to show raw filing text + links to the next concall transcript after the change

### Flow 5 — Financials Comparison
1. Analyst selects 2–5 companies for comparison
2. Selects metric (Revenue / EBITDA / EBITDA Margin / PAT) and quarter range
3. Chart renders: line or bar chart, one series per company
4. Restatement flags shown as markers on the chart with tooltip explaining the change
5. Underlying data table downloadable as CSV

---

## 16. Acceptance Criteria

Each criterion below must be verifiable by running a test. The agent must implement tests for each.

| ID | Criterion | Verification |
|---|---|---|
| AC-01 | Keyword search returns results in < 2s for any query across full corpus | Automated latency test |
| AC-02 | RAG Q&A returns first token in < 5s | Automated timing test |
| AC-03 | Financial extractor reproduces verified Laurus Labs 3QFY25 figures exactly (Revenue 1547 Cr, EBITDA 278 Cr, PAT 152 Cr) | Unit test with fixture |
| AC-04 | Speaker attribution correctly classifies management vs analyst for > 85% of turns in test set | Evaluation against labeled test set (50 turns) |
| AC-05 | BSE poller correctly parses filing metadata from real API response | Integration test with live API |
| AC-06 | Document classifier correctly classifies all 5 document types on test set of 25 filings | Evaluation against labeled test set |
| AC-07 | Management change extractor correctly extracts person, role, change type, effective date for > 92% of test filings | Evaluation against labeled test set (25 filings) |
| AC-08 | Financial consistency checks correctly flag known-bad test cases | Unit test with 10 crafted bad fixtures |
| AC-09 | All 11 RAG benchmark questions return factually correct answers | Manual validation by domain expert |
| AC-10 | Pipeline is fully idempotent — re-running backfill on already-processed data produces no duplicates | Integration test |
| AC-11 | Coverage dashboard correctly reflects true extraction status for all 30 companies | Integration test |
| AC-12 | Restatement detection fires when a previously stored value changes by > 5% on re-ingestion | Unit test with fixture |

---

*Document Owner: Research Team*
*Last Updated: April 2026*
*Version: 2.0 — includes toolchain, user flows, acceptance criteria*
