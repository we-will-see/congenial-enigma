...
| Company Page | Timeline of events, event summaries, management changes log, link to full transcript viewer |
...
│   └── mgmt_change_parser.py   # LLM-based structured extraction of management change filings
├── indexing/
...
- Build `text_extractor.py`, `transcript_parser.py`, `press_release_parser.py`, and `mgmt_change_parser.py`
- Populate Postgres `companies`, `events`, `documents`, `turns`, `press_release_sections`, and `management_changes` tables
- Build `coverage_report.py` to monitor gaps

Success metric: > 85% of expected filings downloaded and turns extracted for Nifty 100 companies in universe. Management changes table populated with > 90% of known KMP changes for at least 5 test companies.
...
Success metric: RAG correctly answers all 8 benchmark questions listed in section 6.4.3 with accurate citations. Management changes view correctly shows full KMP/Board history for at least 5 spot-checked companies.
...
| LLM API costs (RAG + mgmt change extraction at scale) | Low at MVP | Medium | Cache management change extractions; rate limit Q&A queries |
...
| RAG Q&A accuracy (benchmark questions) | 8 / 8 correct with accurate citations |
